...
if (0) timeit(100, function(cb) {
    var x = 0;
    repeatUntil(function(next) { next(null, ++x >= 1e4) }, cb)
    // repeatUntil without a test callout:
    // nextTick: 23m/s len 10, 27m/s len 20, 30m/s len 40
    // with a tested-for (not provided) test callout:
    // nextTick: 21m/s len 10, 27m/s len 40
}, function() {
    console.log("AR: Done.")
});
...
if (1) timeit(100, function(cb) {
    var x = 0;
    repeatUntil(
        function(next) { next(null, ++x >= 1e4) },
        function(err, done) { return (err || err === false || done) },
        cb)
    // setImmediate: 10m/s len 10
    // repeatUntil *with* a test callout:
    // nextTick: 21m/s len 10, 23m/s len 20, 25m/s len 40
}, function() {
    console.log("AR: Done.")
});
